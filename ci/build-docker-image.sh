...
dockerFile="$1"
dockerTag="$2"
githubTag="ghcr.io/bananocoin/${dockerTag#*/}"
shift
shift
...
"$scripts"/custom-timeout.sh 30 docker build "$@" --build-arg REPOSITORY=bananocoin -f "${dockerFile}" -t "${githubTag}" --cache-from "${githubTag}" .
