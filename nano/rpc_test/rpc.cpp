...
TEST (rpc, history)
{
	...
	ASSERT_EQ (nano::MBAN_ratio.convert_to<std::string> (), std::get<2> (history_l[0]));
	ASSERT_EQ (5, history_l.size ());
	...
	ASSERT_EQ (nano::MBAN_ratio.convert_to<std::string> (), std::get<2> (history_l[1]));
...
TEST (rpc, history_pruning)
{
	...
	auto usend (std::make_shared<nano::state_block> (nano::dev::genesis->account (), receive->hash (), nano::dev::genesis->account (), nano::dev::constants.genesis_amount - nano::MBAN_ratio, nano::dev::genesis->account (), nano::dev::genesis_key.prv, nano::dev::genesis_key.pub, *node0->work_generate_blocking (receive->hash ())));
...
TEST (rpc, process_subtype_send)
{
	...
	nano::keypair key;
	auto latest (node1->latest (nano::dev::genesis_key.pub));
	nano::state_block send (nano::dev::genesis->account (), latest, nano::dev::genesis->account (), nano::dev::constants.genesis_amount - nano::MBAN_ratio, key.pub, nano::dev::genesis_key.prv, nano::dev::genesis_key.pub, *node1->work_generate_blocking (latest));
	boost::property_tree::ptree request;
	request.put ("action", "process");
	std::string json;
	send.serialize_json (json);
...
TEST (rpc, process_subtype_open)
{
	...
	nano::keypair key;
	auto latest (node1->latest (nano::dev::genesis_key.pub));
	auto send = std::make_shared<nano::state_block> (nano::dev::genesis->account (), latest, nano::dev::genesis->account (), nano::dev::constants.genesis_amount - nano::MBAN_ratio, key.pub, nano::dev::genesis_key.prv, nano::dev::genesis_key.pub, *node1->work_generate_blocking (latest));
	ASSERT_EQ (nano::process_result::progress, node1->process (*send).code);
	ASSERT_EQ (nano::process_result::progress, node2.process (*send).code);
	auto [rpc, rpc_ctx] = add_rpc (system, node1);
	node1->scheduler.manual (send);
	nano::state_block open (key.pub, 0, key.pub, nano::MBAN_ratio, send->hash (), key.prv, key.pub, *node1->work_generate_blocking (key.pub));
	boost::property_tree::ptree request;
	request.put ("action", "process");
	std::string json;
...
TEST (rpc, process_subtype_receive)
{
	...
	auto & node2 = *system.add_node ();
	auto latest (node1->latest (nano::dev::genesis_key.pub));
	auto send = std::make_shared<nano::state_block> (nano::dev::genesis->account (), latest, nano::dev::genesis->account (), nano::dev::constants.genesis_amount - nano::MBAN_ratio, nano::dev::genesis_key.pub, nano::dev::genesis_key.prv, nano::dev::genesis_key.pub, *node1->work_generate_blocking (latest));
	ASSERT_EQ (nano::process_result::progress, node1->process (*send).code);
	ASSERT_EQ (nano::process_result::progress, node2.process (*send).code);
	auto [rpc, rpc_ctx] = add_rpc (system, node1);
	node1->scheduler.manual (send);
...
TEST (rpc, process_ledger_insufficient_work)
{
	...
	nano::state_block send (nano::dev::genesis->account (), latest, nano::dev::genesis->account (), nano::dev::constants.genesis_amount - nano::MBAN_ratio, nano::dev::genesis_key.pub, nano::dev::genesis_key.prv, nano::dev::genesis_key.pub, system.work_generate_limited (latest, min_difficulty, max_difficulty));
...
TEST (rpc, work_generate_block_high)
{
	...
	auto [rpc, rpc_ctx] = add_rpc (system, node);
	nano::keypair key;
	nano::state_block block (key.pub, 0, nano::dev::genesis_key.pub, nano::MBAN_ratio, 123, key.prv, key.pub, *node->work_generate_blocking (key.pub));
...
TEST (rpc, work_generate_block_low)
{
	...
	auto [rpc, rpc_ctx] = add_rpc (system, node);
	nano::keypair key;
	nano::state_block block (key.pub, 0, nano::dev::genesis_key.pub, nano::MBAN_ratio, 123, key.prv, key.pub, 0);
...
TEST (rpc, work_generate_block_ledger_epoch_2)
{
	...
	nano::keypair key;
	system.wallet (0)->insert_adhoc (nano::dev::genesis_key.prv);
	auto send_block (system.wallet (0)->send_action (nano::dev::genesis_key.pub, key.pub, nano::MBAN_ratio));
	ASSERT_NE (nullptr, send_block);
	nano::state_block block (key.pub, 0, nano::dev::genesis_key.pub, nano::MBAN_ratio, send_block->hash (), key.prv, key.pub, 0);
...
TEST (rpc, block_count_pruning)
{
	...
	auto latest (node1->latest (nano::dev::genesis_key.pub));
	auto send1 (std::make_shared<nano::send_block> (latest, nano::dev::genesis_key.pub, nano::dev::constants.genesis_amount - nano::MBAN_ratio, nano::dev::genesis_key.prv, nano::dev::genesis_key.pub, *node1->work_generate_blocking (latest)));
...
TEST (rpc, json_block_input)
{
	...
	nano::keypair key;
	system.wallet (0)->insert_adhoc (key.prv);
	nano::state_block send (nano::dev::genesis->account (), node1->latest (nano::dev::genesis_key.pub), nano::dev::genesis->account (), nano::dev::constants.genesis_amount - nano::MBAN_ratio, key.pub, nano::dev::genesis_key.prv, nano::dev::genesis_key.pub, 0);
	auto [rpc, rpc_ctx] = add_rpc (system, node1);
	boost::property_tree::ptree request;
...
TEST (rpc, blocks_info_subtype)
{
	...
	system.wallet (0)->insert_adhoc (nano::dev::genesis_key.prv);
	system.wallet (0)->insert_adhoc (key.prv);
	auto send (system.wallet (0)->send_action (nano::dev::genesis_key.pub, nano::dev::genesis_key.pub, nano::MBAN_ratio));
	ASSERT_NE (nullptr, send);
	auto receive (system.wallet (0)->receive_action (send->hash (), key.pub, nano::MBAN_ratio, send->link ().as_account ()));
	ASSERT_NE (nullptr, receive);
...
TEST (rpc, block_create_state)
{
	...
	request.put ("balance", (nano::dev::constants.genesis_amount - nano::MBAN_ratio).convert_to<std::string> ());
...
TEST (rpc, block_create_state_open)
{
	...
	request.put ("representative", nano::dev::genesis_key.pub.to_account ());
	request.put ("balance", nano::MBAN_ratio.convert_to<std::string> ());
...
TEST (rpc, block_create_receive_epoch_v2)
{
	...
	auto send_block (system.wallet (0)->send_action (nano::dev::genesis_key.pub, key.pub, nano::MBAN_ratio));
	ASSERT_NE (nullptr, send_block);
	nano::state_block open (key.pub, 0, nano::dev::genesis_key.pub, nano::MBAN_ratio, send_block->hash (), key.prv, key.pub, *node->work_generate_blocking (key.pub));
	ASSERT_EQ (nano::process_result::progress, node->process (open).code);
	ASSERT_NE (nullptr, system.upgrade_genesis_epoch (*node, nano::epoch::epoch_2));
	auto send_block_2 (system.wallet (0)->send_action (nano::dev::genesis_key.pub, key.pub, nano::MBAN_ratio));
	auto [rpc, rpc_ctx] = add_rpc (system, node);
	boost::property_tree::ptree request;
	request.put ("action", "block_create");
	request.put ("type", "state");
	...
	request.put ("representative", nano::dev::genesis_key.pub.to_account ());
	request.put ("balance", (2 * nano::MBAN_ratio).convert_to<std::string> ());
...
TEST (rpc, online_reps)
{
	...
	ASSERT_EQ (node2->online_reps.online (), nano::dev::constants.genesis_amount - nano::MBAN_ratio);
...
TEST (rpc, confirmation_height_currently_processing)
{
	...
	auto previous_genesis_chain_hash = node->latest (nano::dev::genesis_key.pub);
	{
		auto transaction = node->store.tx_begin_write ();
		nano::keypair key1;
		nano::send_block send (previous_genesis_chain_hash, key1.pub, nano::dev::constants.genesis_amount - nano::MBAN_ratio - 1, nano::dev::genesis_key.prv, nano::dev::genesis_key.pub, *system.work.generate (previous_genesis_chain_hash));
...
TEST (rpc, confirmation_history)
{
	...
	auto block (system.wallet (0)->send_action (nano::dev::genesis_key.pub, key.pub, nano::MBAN_ratio));
	...
	debug_assert (tally_num == nano::dev::constants.genesis_amount || tally_num == (nano::dev::constants.genesis_amount - nano::MBAN_ratio));
	system.stop ();
}
...
TEST (rpc, confirmation_history_hash)
{
	...
	system.wallet (0)->insert_adhoc (nano::dev::genesis_key.prv);
	ASSERT_TRUE (node->active.list_recently_cemented ().empty ());
	auto send1 (system.wallet (0)->send_action (nano::dev::genesis_key.pub, key.pub, nano::MBAN_ratio));
	auto send2 (system.wallet (0)->send_action (nano::dev::genesis_key.pub, key.pub, nano::MBAN_ratio));
	auto send3 (system.wallet (0)->send_action (nano::dev::genesis_key.pub, key.pub, nano::MBAN_ratio));
	...
	nano::amount tally_num;
	tally_num.decode_dec (tally);
	debug_assert (tally_num == nano::dev::constants.genesis_amount || tally_num == (nano::dev::constants.genesis_amount - nano::MBAN_ratio) || tally_num == (nano::dev::constants.genesis_amount - 2 * nano::MBAN_ratio) || tally_num == (nano::dev::constants.genesis_amount - 3 * nano::MBAN_ratio));
	system.stop ();
}
...
TEST (rpc, account_lazy_start)
{
	...
	auto send1 (std::make_shared<nano::state_block> (nano::dev::genesis_key.pub, nano::dev::genesis->hash (), nano::dev::genesis_key.pub, nano::dev::constants.genesis_amount - nano::MBAN_ratio, key.pub, nano::dev::genesis_key.prv, nano::dev::genesis_key.pub, *system.work.generate (nano::dev::genesis->hash ())));
	ASSERT_EQ (nano::process_result::progress, node1->process (*send1).code);
