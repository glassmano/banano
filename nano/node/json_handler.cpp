	...
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			if (receivable)
			{
				...
				response_l.put ("pending_decimal", convert_raw_to_dec (account_receivable.convert_to<std::string> ()));
				response_l.put ("receivable", account_receivable.convert_to<std::string> ());
				response_l.put ("receivable_decimal", convert_raw_to_dec (account_receivable.convert_to<std::string> ()));
				...
				if (include_confirmed)
				{
					...
					response_l.put ("confirmed_pending_decimal", convert_raw_to_dec (account_receivable.convert_to<std::string> ()));
				}
			}
		}
	}
	...
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			...
			if (amount)
			{
				...
				response_l.put ("amount_decimal", convert_raw_to_dec (amount.value ().number ().convert_to<std::string> ()));
			}
			auto balance = block->balance ();
			response_l.put ("balance", balance.number ().convert_to<std::string> ());
			response_l.put ("balance_decimal", convert_raw_to_dec (balance.number ().convert_to<std::string> ()));
			...
			if (!hash.decode_hex (hash_text))
			{
				...
				if (block != nullptr)
				{
					...
					if (amount)
					{
						...
						entry.put ("amount_decimal", convert_raw_to_dec (amount.value ().number ().convert_to<std::string> ()));
					}
					auto balance = block->balance ();
					entry.put ("balance", balance.number ().convert_to<std::string> ());
					entry.put ("balance_decimal", convert_raw_to_dec (balance.number ().convert_to<std::string> ()));
...
public:
	...
	void send_block (nano::send_block const & block_a)
	{
		...
		if (amount)
		{
			...
			tree.put ("amount_decimal", convert_raw_to_dec (amount.value ().number ().convert_to<std::string> ()));
		}
		if (raw)
		{
	...
	void state_block (nano::state_block const & block_a)
	{
		...
		else if (balance < previous_balance.number ())
		{
			...
			tree.put ("amount_decimal", convert_raw_to_dec ((previous_balance.number () - balance).convert_to<std::string> ()));
		}
		else
		{
			...
			else
			{
				...
				tree.put ("amount_decimal", convert_raw_to_dec ((balance - previous_balance.number ()).convert_to<std::string> ()));
			}
		}
	}
			...
			for (auto i (node.store.account.begin (transaction, start)), n (node.store.account.end (transaction)); i != n && accounts.size () < count; ++i)
			{
				...
				if (info.modified >= modified_since && (receivable || info.balance.number () >= threshold.number ()))
				{
					...
					if (receivable)
					{
						...
						response_a.put ("pending_decimal", convert_raw_to_dec (account_receivable.convert_to<std::string> ()));
						response_a.put ("receivable", account_receivable.convert_to<std::string> ());
						response_a.put ("receivable_decimal", convert_raw_to_dec (account_receivable.convert_to<std::string> ()));
					}
...
void nano::json_handler::wallet_info ()
{
	...
	if (!ec)
	{
		...
		response_l.put ("pending_decimal", convert_raw_to_dec (receivable.convert_to<std::string> ()));
		response_l.put ("receivable", receivable.convert_to<std::string> ());
		response_l.put ("receivable_decimal", convert_raw_to_dec (receivable.convert_to<std::string> ()));
	...
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end (transaction)); i != n; ++i)
		{
			...
			if (balance >= threshold.number ())
			{
				...
				entry.put ("pending_decimal", convert_raw_to_dec (receivable.convert_to<std::string> ()));
				entry.put ("receivable", receivable.convert_to<std::string> ());
				entry.put ("receivable_decimal", convert_raw_to_dec (receivable.convert_to<std::string> ()));
			...
			if (info)
			{
				if (info->modified >= modified_since)
				{
					...
					if (receivable)
					{
						...
						entry.put ("pending_decimal", convert_raw_to_dec (account_receivable.convert_to<std::string> ()));
						entry.put ("receivable", account_receivable.convert_to<std::string> ());
						entry.put ("receivable_decimal", convert_raw_to_dec (account_receivable.convert_to<std::string> ()));
					}
