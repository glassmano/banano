...
TEST (bootstrap_processor, lazy_hash_pruning)
{
	...
	// send MBAN_ratio raw from genesis to genesis
	auto send1 = builder
				 .account (nano::dev::genesis_key.pub)
				 .previous (nano::dev::genesis->hash ())
				 .representative (nano::dev::genesis_key.pub)
	...
	// send MBAN_ratio from genesis to key1 and genesis rep back to genesis account
	auto send2 = builder
				 .make_block ()
				 .account (nano::dev::genesis_key.pub)
	...
	// send MBAN_ratio raw, all available balance, from key1 to key2
	auto send3 = builder
				 .make_block ()
				 .account (key1.pub)
...
TEST (bootstrap_processor, lazy_destinations)
{
	...
	// send MBAN_ratio raw from genesis to key1
	auto send1 = builder
				 .state ()
				 .account (nano::dev::genesis_key.pub)
				 .previous (nano::dev::genesis->hash ())
	...
	// send MBAN_ratio raw from genesis to key2
	auto send2 = builder
				 .state ()
				 .account (nano::dev::genesis_key.pub)
				 .previous (send1->hash ())
